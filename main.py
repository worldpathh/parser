...
WARMUP_DELAY_MIN = getattr(_cfg, "WARMUP_DELAY_MIN", 40)
WARMUP_DELAY_MAX = getattr(_cfg, "WARMUP_DELAY_MAX", 60)
...
async def run_parser(
    ...
    try:
        ...
        logger.info(
            ...
            "(с паузой %s–%s сек между аккаунтами)",
            WARMUP_DELAY_MIN,
            WARMUP_DELAY_MAX,
        )
        for idx, client in enumerate(clients):
            ...
            if idx > 0:
                if WARMUP_DELAY_MIN == WARMUP_DELAY_MAX:
                    pause = float(WARMUP_DELAY_MIN)
                else:
                    low = min(WARMUP_DELAY_MIN, WARMUP_DELAY_MAX)
                    high = max(WARMUP_DELAY_MIN, WARMUP_DELAY_MAX)
